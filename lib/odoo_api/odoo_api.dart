...
class OdooClient extends OdooConnector {
  ...
  Future<AuthenticateCallback> authenticate(
    ...
    return new AuthenticateCallback(!response.hasError() && !session.hasError(),
        response, session.getSessionId());
  }

  // Read records with given ids and fields
  Future<OdooResponse> read(String model, List<int> ids, List<String> fields,
      {dynamic kwargs, Map<dynamic, dynamic>? context}) async {
  ...
  Future<OdooResponseData> callControllerBackSlash(String path, Map params) async {
    return await callRequestBackSlash(createPath(path), createPayload(params));
  }

  // Call json controller for ICPD
  Future<icpdOdooResponse> callControllerICPD(String path, Map params) async {
    return await callRequestICPD(createPath(path), createPayload(params));
  }

  Future<icpdOdooResponse> callControllerICPDBackSlash(
      String path, Map params) async {
    return await callRequestICPDBackSlash(
        createPath(path), createPayload(params));
  }
}
